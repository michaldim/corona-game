    ...
    const move = (figure) => {
        ...
        const figureMovement = (randomIntX, randomIntY) => {
            ...
            if (currentFigure.offsetTop <= 1) { //offsetTop shows the location comparing to the father (the body). We need that offsetTop will be 0 or 1 (and not only offsetTop=0), because sometimes the figures do 2 steps (2 pixels) at a time
                currentFigure.style.top = ((ourViewPortHeight - 2) + 'px');//Minus 2, because the figures will be stucked if we will send them to ourViewPortHeightMinus1 or ourViewPortHeight
                currentFigure.style.left = (body.clientWidth - parseInt(currentFigure.style.left)) + 'px';
            } 
            else if (currentFigure.offsetTop >= (body.clientHeight - 1)) {
                currentFigure.style.top = '2px';
                currentFigure.style.left = (body.clientWidth - parseInt(currentFigure.style.left)) + 'px';
            } 
            else if (currentFigure.offsetLeft <= 1) {
                currentFigure.style.left = ((ourViewPortWidth -2) + 'px');
                currentFigure.style.top = (body.clientHeight - parseInt(currentFigure.style.top)) + 'px';
            } 
            else if (currentFigure.offsetLeft >= (body.clientWidth - 1)) {
                currentFigure.style.left = '2px';
                currentFigure.style.top = (body.clientHeight - parseInt(currentFigure.style.top)) + 'px';
            } 
            //now 4 rules for the corners' cases:
            else if ((currentFigure.offsetTop <= 1) && (currentFigure.offsetLeft >= (body.clientWidth - 1))) {
                currentFigure.style.top = ((ourViewPortHeight - 2) + 'px');
                currentFigure.style.left = '2px';
            }
            ...
            else if ((currentFigure.offsetTop >= (body.clientHeight - 1)) && (currentFigure.offsetLeft <= 1)) {
                currentFigure.style.top = '2px';
                currentFigure.style.left = ((ourViewPortWidth -2) + 'px');
            }
            else if ((currentFigure.offsetTop >= (body.clientHeight - 1)) && (currentFigure.offsetLeft >= (body.clientWidth - 1))) {
                currentFigure.style.top = '2px';
                currentFigure.style.left = '2px';
            }
